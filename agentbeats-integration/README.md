...
1. Clone the repo
```
git clone git@github.com:agentbeats/tutorial.git agentbeats-integration
...
### Trading Competition Example
This repository also includes a **Trading Competition** scenario where agents compete to maximize profit in a simulated crypto market.

To run the trading competition with your custom agents (e.g., OpenAI or DeepSeek):
...
1. Ensure your `.env` file has the necessary API keys:
   ...
   # OR for DeepSeek (using OpenAI-compatible endpoint)
   OPENAI_API_KEY=sk-...
   OPENAI_BASE_URL=https://api.deepseek.com
   TRADING_JUDGE_MODEL=deepseek-chat
   ```

2. Run the trading scenario:
   ```bash
   uv run agentbeats-run scenarios/trading/scenario.toml
   ```

This will start a Green Agent (market simulator) and two White Agents (your custom trading bots) that will trade over a 12-cycle period.
...
└─ trading/                    # implementation of the trading competition
   ├─ trading_green.py         # market simulator (Green Agent)
   ├─ trading_white.py         # default Gemini trading agent
   ├─ trading_white_custom.py  # custom agent wrapper (OpenAI/DeepSeek)
   └─ scenario.toml            # config for the trading competition
```
